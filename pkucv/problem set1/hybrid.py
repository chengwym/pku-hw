...
def cross_correlation_2d(image, kernal, stride=1, zero_padding=True):
    """
    ...
    for d in range(D):
        for i in range(_w, W-_w, stride):
            for j in range(_h, H-_h, stride):
...
def convolve_2(image, kernal, stride=1):
    """
