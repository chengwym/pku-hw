 ...
 */
void command_kill(struct cmdline_tokens tok) {
    struct job_t *job;
    int is_negi=false;
    if ((job=Getjob(tok,&is_negi))==NULL) return;
    sio_put("%d %d\n",getpid(),job->pid);
    if (!is_negi){
...
void command_job(struct cmdline_tokens tok) {
    if (tok.outfile!=NULL){
        int fid=Open(tok.outfile,O_WRONLY);
...
void command_FB(struct cmdline_tokens tok) {
    struct job_t *job;
