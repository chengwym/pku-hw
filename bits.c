...
int byteSwap(int x, int n, int m) {
    ...
    int seg1 = x & tool1;
    int seg2 = x & tool2;
    int tool3 = ~(tool1 | tool2);
    int seg3 = x & tool3;
    seg1 = ((seg1 >> (n << 3)) << (m << 3)) & tool2;
    seg2 = ((seg2 >> (m << 3)) << (n << 3)) & tool1; 
...
int divpwr2(int x, int n) {
    int sign = x >> 31;
    int flag = ((x>>n)&(~sign)) | (((x+(1<<n)+(~0))>>n)&sign);
    return flag;
}
/* 
...
int satMul3(int x) {
    int x1 = (x << 1) + x;
    int flag1 = ((x1 ^ x)>>31) | (((x<<1)^x)>>31);
    int flag2 = x >> 31;
    int TMin = 1 << 31;
    int TMax = 0xff | (0xff << 8) |(0xff<<16)|(0x7f<<24);
    return (x1 & ~flag1) | (flag1 & flag2 & TMin)| (flag1 & ~flag2 & TMax);
}
/* 
...
int isLessOrEqual(int x, int y) {
  ...
  result = result >> 31;
  flag = (!(y>>31))&((x>>31));
  overflow = !(((y^x)&(y^result))>>31);
  return ((!result)&overflow)|flag;
}
/*
...
unsigned float_i2f(int x) {
  ...
  if ((x & 0x000001ff) == 0x180 || (x & 0xff) > 0x80) {
    a = 1;
  }
  f = ((x & 0x7fffffffu) >> 8) + a;
  return s + ((e + 127) << 23) + f;
